...
while ciclo == True:
    scelta = input("Vuoi fare una ricerca(r), Inserire i testi(t), inserire un url(u) o fermare il programma(s): ")
    if scelta == "r":
        ...
        df = pd.read_csv('output/scraped.csv', index_col=0)
        ...
        texts=[]
                ...
                # Omit the first 11 lines and the last 4 lines, then replace -, _, and #update with spaces.
        texts.append((contenuto_formattato[:].replace('-',' ').replace('_', ' ').replace('#update',''), contenuto_formattato))
